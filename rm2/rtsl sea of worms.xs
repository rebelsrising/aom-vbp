...
void main() {
	...
	// RM X Setup.
	rmxInit("Sea of Worms (RTSL)", false, false, false);
	...
	int farAvoidSettlement = rmCreateTypeDistanceConstraint("TCs avoid TCs by long distance", "AbstractSettlement", 70.0);
	...
	if (boarNumber < 0.3) rmAddObjectDefItem(closeBoarID, "boar", 3, 4.0);
	else if (boarNumber < 0.6) rmAddObjectDefItem(closeBoarID, "boar", 3, 4.0);
	...
	rmSetObjectDefMinDistance(farGoldID, 100.0);
	rmSetObjectDefMaxDistance(farGoldID, 120.0);
   ...
   for(i=1; <cNumberPlayers)
   {
	...
	id = rmAddFairLoc("Settlement", false, true, 70, 90, 60, 10);
	rmAddFairLocConstraint(id, centerConstraint);

	id = rmAddFairLoc("Settlement", true, false, 100, 120, 60, 10);
	rmAddFairLocConstraint(id, centerConstraint);
	...
	// Gold
	//   for(i=1; <cNumberPlayers)
	rmPlaceObjectDefPerPlayer(mediumGoldID, true, rmRandInt(1, 2));
