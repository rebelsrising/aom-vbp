...
void main() {
	...
	// RM X Setup.
	rmxInit("Sea of Worms (ESO)", false, false, false);
	...
	if (boarNumber < 0.3) rmAddObjectDefItem(closeBoarID, "boar", 1, 4.0);
	else if (boarNumber < 0.6) rmAddObjectDefItem(closeBoarID, "boar", 2, 4.0);
	else rmAddObjectDefItem(closeBoarID, "boar", 3, 6.0);
	...
	if (predatorSpecies < 0.5) rmAddObjectDefItem(farPredatorID, "bear", 1, 4.0);
	else rmAddObjectDefItem(farPredatorID, "polar bear", 1, 4.0);
	...
	if (bonusChance < 0.5) rmAddObjectDefItem(bonusHuntableID, "boar", rmRandInt(1, 3), 4.0);
	else rmAddObjectDefItem(bonusHuntableID, "elk", rmRandInt(2, 8), 4.0);
	...
	if (hand == 1) rmSetPlacementSection(0.55, 0.15);
	else rmSetPlacementSection(0.05, 0.65);
	...
	if (hand == 1) rmSetAreaLocation(centerID, 0.6, 0.4);
	else rmSetAreaLocation(centerID, 0.4, 0.6);
	...
	if (hand == 1) rmSetAreaLocation(gulfID, 0.9, 0.25);
	else rmSetAreaLocation(gulfID, 0.1, 0.75);
	...
	if (cNumberNonGaiaPlayers < 4) {
		if (rmRandFloat(0, 1) < 0.5) numIsland = rmRandInt(1, 3);
	}
	else if (cNumberNonGaiaPlayers < 7) numIsland = rmRandInt(2, 6);
	else numIsland = rmRandInt(3, 7);
	for (i = 1; < numIsland) {
   ...
   for(i=1; <cNumberPlayers)
   {
	...
	for (i = 0; < numTries) {
		...
		if (rmRandFloat(0.0, 1.0) < 0.5) rmSetAreaTerrainType(elevID, "SnowB");
	...
	if (rmPlaceFairLocs()) {
		...
		for (i = 1; < cNumberPlayers) {
			for (j = 0; < rmGetNumberFairLocs(i)) {
				if (rmGetPlayerCulture(i) == cCultureGreek) rmPlaceObjectDefAtLoc(fortressID, i, rmFairLocXFraction(i, j), rmFairLocZFraction(i, j), 1);
				else if (rmGetPlayerCulture(i) == cCultureEgyptian) rmPlaceObjectDefAtLoc(migdolID, i, rmFairLocXFraction(i, j), rmFairLocZFraction(i, j), 1);
				else if (rmGetPlayerCulture(i) == cCultureNorse) rmPlaceObjectDefAtLoc(hillFortID, i, rmFairLocXFraction(i, j), rmFairLocZFraction(i, j), 1);
				else if (rmGetPlayerCulture(i) == cCultureAtlantean) rmPlaceObjectDefAtLoc(palaceID, i, rmFairLocXFraction(i, j), rmFairLocZFraction(i, j), 1);
			}
		}
	}
	...
	if (rmRandFloat(0, 1) < 0.5) rmAddObjectDefItem(sharkID, "orca", 1, 0.0);
	else rmAddObjectDefItem(sharkID, "whale", 1, 0.0);
	...
	//Bonus Relics
	if (rmRandFloat(0, 1) < 0.33) for (i = 1; < rmRandInt(1, 2)) rmPlaceObjectDefInRandomAreaOfClass(bonusRelicID, 0, classBonusIsland);
	...
	for (i = 0; < forestCount) {
		...
		if (rmRandFloat(0.0, 1.0) < 0.25) rmSetAreaForestType(forestID, "snow pine forest");
		else rmSetAreaForestType(forestID, "pine forest");
