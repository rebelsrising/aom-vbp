...
void main() {
   ...
   rmSetTriggerEffectParam("Status", "4"); */
	...
	// RM X Setup.
	rmxInit("Sudden Death (ESO)", false, false, false);
	...
	if (elkNumber < 0.3) rmAddObjectDefItem(closeElkID, "elk", 4, 2.0);
	else if (elkNumber < 0.6) rmAddObjectDefItem(closeElkID, "elk", 6, 2.0);
	else rmAddObjectDefItem(closeElkID, "elk", 7, 2.0);
	...
	if (bonusChance < 0.5) rmAddObjectDefItem(bonusHuntableID, "boar", 3, 2.0);
	else rmAddObjectDefItem(bonusHuntableID, "deer", 7, 6.0);
	...
	if (cNumberNonGaiaPlayers == 2) rmPlacePlayersLine(0.15, 0.15, 0.85, 0.85, 0, 0);
	...
	else rmPlacePlayersCircular(0.35, 0.4, rmDegreesToRadians(5.0));
