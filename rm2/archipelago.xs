...
void main() {
	...
	// RM X Setup.
	rmxInit("Archipelago (ESO)", false, false, false);
	...
	if (bonusChance < 0.5) rmAddObjectDefItem(bonusHuntableID, "boar", 2, 4.0);
	else rmAddObjectDefItem(bonusHuntableID, "deer", 6, 8.0);
	...
	if (cNumberNonGaiaPlayers < 4) rmPlacePlayersCircular(0.23, 0.25, rmDegreesToRadians(5.0));
	else if (cNumberNonGaiaPlayers < 9) rmPlacePlayersCircular(0.23, 0.28, rmDegreesToRadians(5.0));
	else rmPlacePlayersCircular(0.30, 0.35, rmDegreesToRadians(5.0));

	// Set up player areas.
	float playerFraction = rmAreaTilesToFraction(4500);
	if (cNumberNonGaiaPlayers < 4) playerFraction = rmAreaTilesToFraction(4200);
	...
	for (i = 1; < cNumberPlayers) {
		...
		if (cNumberNonGaiaPlayers < 3) rmAddAreaConstraint(id, playerConstraint);
