...
void main() {
	...
	// RM X Setup.
	rmxInit("Ghost Land", false, false, false);
	...
	if (rmRandFloat(0, 1) < 0.6) rmAddObjectDefItem(closeBoarID, "boar", rmRandInt(1, 3), 4.0);
	else rmAddObjectDefItem(closeBoarID, "aurochs", rmRandInt(1, 2), 4.0);
	...
	rmSetAreaTerrainType(centerID, "SnowGrass25");
	rmAddAreaTerrainLayer(centerID, "SnowGrass", 6, 10);
	rmAddAreaTerrainLayer(centerID, "SnowGrass", 0, 6);
	...
	for (i = 1; < 10) {
		...
		rmSetAreaTerrainType(icePatch, "SnowGrass25");
		rmAddAreaTerrainLayer(icePatch, "SnowGrass25", 0, 3);
	...
	id = rmAddFairLoc("Settlement", true, false, 80, 100, 60, 10);
	/* forward outside */
	rmAddFairLocConstraint(id, settlementCenterConstraint);
	...
	// Draw cliffs
	int cliffNumber = 0;
	if (cNumberNonGaiaPlayers < 4) cliffNumber = 4;
	else if (cNumberNonGaiaPlayers < 9) cliffNumber = 6;
	else cliffNumber = 9;

	for (i = 0; < cliffNumber) {
		int cliffID = rmCreateArea("cliff" + i);
		rmSetAreaWarnFailure(cliffID, false);
		if (cNumberNonGaiaPlayers < 4) rmSetAreaSize(cliffID, rmAreaTilesToFraction(200), rmAreaTilesToFraction(300));
		else rmSetAreaSize(cliffID, rmAreaTilesToFraction(400), rmAreaTilesToFraction(600));
	...
	for (i = 1; < cNumberPlayers * 10) {
		...
		if (rmRandFloat(0, 1) < 0.5) rmSetAreaTerrainType(patch2ID, "SnowA");
		else rmSetAreaTerrainType(patch2ID, "SnowSand25");
