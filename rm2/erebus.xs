...
void main() {
	...
	// RM X Setup.
	rmxInit("Erebus (ESO)", false, false, false);
	...
	if (predatorSpecies < 0.5) rmAddObjectDefItem(farPredatorID, "serpent", 4, 4.0);
	else rmAddObjectDefItem(farPredatorID, "serpent", 2, 4.0);
	...
	if (predatorSpecies < 0.5) rmAddObjectDefItem(farPredator2ID, "shade of erebus", 4, 4.0);
	else rmAddObjectDefItem(farPredator2ID, "shade of erebus", 2, 4.0);
	...
	if (cNumberNonGaiaPlayers > 8) rmPlacePlayersCircular(0.35, 0.45, rmDegreesToRadians(5.0));
	else rmPlacePlayersCircular(0.3, 0.4, rmDegreesToRadians(5.0));

	int shallowsID = rmCreateConnection("shallows");
	if (cNumberNonGaiaPlayers < 6) rmSetConnectionType(shallowsID, cConnectPlayers, true, 1.0);
	else rmSetConnectionType(shallowsID, cConnectPlayers, false, 1.0);
	...
	if (cNumberNonGaiaPlayers < 4) rmSetConnectionType(extraShallowsID, cConnectAreas, true, 1.0);
	else rmSetConnectionType(extraShallowsID, cConnectAreas, false, 0.75);
   ...
   rmAddConnectionEndConstraint(extraShallowsID, playerConstraint); */
	...
	for (i = 1; < cNumberPlayers) {
		...
		if (styx == 0) rmAddAreaTerrainLayer(id, "HadesCliff", 0, 2);
	...
	// Settlements.
	id = rmAddFairLoc("Settlement", false, true, 60, 80, 40, 10, true);
	...
	for (j = 1; < cNumberPlayers) {
		for (i = 0; < 7) {
			...
			// Hill trees?
			if (rmRandFloat(0.0, 1.0) < 0.2) rmSetAreaBaseHeight(forestID, rmRandFloat(6.0, 8.0));
	...
	if (styx == 0) rmAddObjectDefConstraint(stalagmiteID, avoidImpassableLand);
	...
	if (styx == 0) rmAddObjectDefConstraint(stalagmite2ID, avoidImpassableLand);
	...
	if (styx == 0) rmAddObjectDefConstraint(stalagmite3ID, avoidImpassableLand);
	...
	if (styx == 0) rmPlaceObjectDefAtLoc(flameID, 0, 0.5, 0.5, 8 * cNumberNonGaiaPlayers);
	...
	if (styx == 0) rmPlaceObjectDefAtLoc(bubbleID, 0, 0.5, 0.5, 3 * cNumberNonGaiaPlayers);
