...
void main() {
	...
	// RM X Setup.
	rmxInit("Marsh (ESO)", false, false, false);
	...
	else if (hippoNumber < 0.6) rmAddObjectDefItem(closeHippoID, "hippo", 3, 4.0);
	else rmAddObjectDefItem(closeHippoID, "water buffalo", 2, 1.0);
	...
	if (predatorSpecies < 0.5) rmAddObjectDefItem(farPredatorID, "crocodile", 1, 4.0);
	else rmAddObjectDefItem(farPredatorID, "crocodile", 2, 2.0);
	...
	if (bonusChance < 0.5) rmAddObjectDefItem(bonusHuntableID, "water buffalo", 2, 2.0);
	else if (bonusChance < 0.75) rmAddObjectDefItem(bonusHuntableID, "deer", rmRandInt(5, 6), 3.0);
	else rmAddObjectDefItem(bonusHuntableID, "hippo", rmRandInt(3, 5), 3.0);
	...
	if (bonusChance < 0.5) rmAddObjectDefItem(bonusHuntableID2, "hippo", 2, 2.0);
	else if (bonusChance < 0.75) {
		rmAddObjectDefItem(bonusHuntableID2, "water buffalo", rmRandInt(3, 4), 3.0);
		if (rmRandFloat(0, 1) < 0.5) rmAddObjectDefItem(bonusHuntableID2, "deer", rmRandInt(2, 4), 3.0);
	}
	else rmAddObjectDefItem(bonusHuntableID2, "deer", rmRandInt(6, 9), 4.0);
	...
	bonusChance = rmRandFloat(0, 1);
	if (bonusChance < 0.5) rmAddObjectDefItem(bonusHuntableID3, "boar", 4, 2.0);
	else if (bonusChance < 0.75) {
		rmAddObjectDefItem(bonusHuntableID3, "boar", 6, 3.0);
		if (rmRandFloat(0, 1) < 0.5) rmAddObjectDefItem(bonusHuntableID3, "crowned crane", rmRandInt(4, 6), 4.0);
	}
	else rmAddObjectDefItem(bonusHuntableID3, "water buffalo", rmRandInt(4, 5), 3.0);
	...
	bonusChance = rmRandFloat(0, 1);
	if (bonusChance < 0.5) rmAddObjectDefItem(bonusHuntableID4, "boar", 4, 2.0);
	else if (bonusChance < 0.75) {
		rmAddObjectDefItem(bonusHuntableID4, "boar", 6, 3.0);
		if (rmRandFloat(0, 1) < 0.5) rmAddObjectDefItem(bonusHuntableID4, "crowned crane", rmRandInt(4, 6), 4.0);
	}
	else rmAddObjectDefItem(bonusHuntableID4, "water buffalo", rmRandInt(2, 3), 3.0);
	...
	rmAddConnectionTerrainReplacement(shallowsID, "RiverMarshA", "MarshE");
	/*RiverMarshA */

	// Create extra connections
	int extraShallowsID = rmCreateConnection("extra shallows");
	if (cNumberPlayers < 5) rmSetConnectionType(extraShallowsID, cConnectAreas, false, 0.75);
	else if (cNumberPlayers < 7) rmSetConnectionType(extraShallowsID, cConnectAreas, false, 0.30);
...
*/
	...
	for (i = 0; < bonusCount) {
		...
		if (rmRandFloat(0.0, 1.0) < 0.70) rmAddAreaConstraint(bonusIslandID, bonusIslandConstraint);
	...
	if (cNumberNonGaiaPlayers < 4) rmBuildConnection(extraShallowsID);
	else if (cNumberNonGaiaPlayers < 6) {
		if (rmRandFloat(0, 1) < 0.5) rmBuildConnection(extraShallowsID);
	}
...
*/
		...
		// Hill trees?
		if (rmRandFloat(0.0, 1.0) < 0.6) rmSetAreaBaseHeight(playerForestID, rmRandFloat(10.0, 12.0));
		...
		if (rmBuildArea(playerForestID) == false) {
			// Stop trying once we fail 3 times in a row.
			playerfailCount++;
			if (playerfailCount == 3) break;
		}
		else playerfailCount = 0;
	}
