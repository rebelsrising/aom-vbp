...
void main() {
	...
	// RM X Setup.
	rmxInit("Mediterranean (ESO)", false, false, false);
	...
	if (rmRandFloat(0, 1) < 0.8) rmAddObjectDefItem(closeChickensID, "chicken", rmRandInt(6, 10), 5.0);
	else rmAddObjectDefItem(closeChickensID, "berry bush", rmRandInt(4, 6), 4.0);
	...
	if (rmRandFloat(0, 1) < 0.7) rmAddObjectDefItem(closeBoarID, "boar", rmRandInt(1, 3), 4.0);
	else rmAddObjectDefItem(closeBoarID, "aurochs", rmRandInt(1, 2), 2.0);
	...
	if (bonusChance < 0.5) rmAddObjectDefItem(bonusHuntableID, "boar", rmRandInt(2, 3), 4.0);
	else if (bonusChance < 0.8) rmAddObjectDefItem(bonusHuntableID, "deer", rmRandInt(6, 8), 8.0);
	else rmAddObjectDefItem(bonusHuntableID, "aurochs", rmRandInt(1, 3), 4.0);
	...
	if (cNumberNonGaiaPlayers < 4) rmPlacePlayersCircular(0.4, 0.43, rmDegreesToRadians(5.0));
	else rmPlacePlayersCircular(0.43, 0.45, rmDegreesToRadians(5.0));
	...
	for (i = 1; < cNumberPlayers) {
		...
		if (cNumberNonGaiaPlayers < 4) rmAddAreaConstraint(id, smallMapPlayerConstraint);
	...
	if (rmRandFloat(0, 1) < 0.5) rmAddObjectDefItem(sharkID, "shark", 1, 0.0);
	...
	for (i = 0; < numTries) {
		...
		if (rmRandFloat(0.0, 1.0) < 0.5) rmSetAreaTerrainType(elevID, "GrassDirt50");
	...
	for (i = 0; < forestCount) {
		...
		if (rmRandFloat(0.0, 1.0) < 0.5) rmSetAreaForestType(forestID, "oak forest");
	...
	if (rmRandFloat(0, 1) < 0.5) rmAddObjectDefItem(deerID, "deer", rmRandInt(1, 2), 1.0);
	else rmAddObjectDefItem(deerID, "aurochs", 1, 0.0);
