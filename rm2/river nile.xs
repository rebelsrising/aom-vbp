...
void main() {
	...
	// RM X Setup.
	rmxInit("River Nile (ESO)", false, false, false);
	...
	if (cNumberTeams > 7) avoidImpassableLand = rmCreateTerrainDistanceConstraint("avoid impassable land by less", "land", false, 6.0);
	int farAvoidImpassableLand = rmCreateTerrainDistanceConstraint("far avoid impassable land", "land", false, 20.0);
	if (cNumberTeams > 7) farAvoidImpassableLand = rmCreateTerrainDistanceConstraint("far avoid impassable land by not so much", "land", false, 8.0);
	...
	if (hippoNumber < 0.3) rmAddObjectDefItem(closeHippoID, "hippo", 1, 1.0);
	else if (hippoNumber < 0.6) rmAddObjectDefItem(closeHippoID, "hippo", 2, 4.0);
	else rmAddObjectDefItem(closeHippoID, "rhinocerous", 1, 1.0);
	...
	if (predatorSpecies < 0.5) rmAddObjectDefItem(farPredatorID, "crocodile", 2, 2.0);
	else rmAddObjectDefItem(farPredatorID, "crocodile", 1, 0.0);
	...
	else if (bonusChance < 0.75) rmAddObjectDefItem(bonusHuntableID, "giraffe", 3, 6.0);
	else rmAddObjectDefItem(bonusHuntableID, "zebra", 6, 8.0);
	...
	if (bonusChance < 0.5) rmAddObjectDefItem(bonusHuntableID2, "water buffalo", 4, 2.0);
	...
	if (cNumberTeams < 3) rmPlacePlayersSquare(0.35, 0.05, 10.0);
	else rmPlacePlayersSquare(0.4, 0.05, 10.0);

	// Build team areas.
	int teamClass = rmDefineClass("teamClass");
	int baseRiverWidth = 0;
	if (cNumberTeams < 4) baseRiverWidth = 45;
	else if (cNumberTeams < 7) baseRiverWidth = 30;
	else baseRiverWidth = 30;
	...
	/*   for(i=1; <cNumberPlayers)
   {
	...
	for (i = 1; < cNumberPlayers) {
		if (rmRandFloat(0.0, 1.0) < 0.5) rmPlaceObjectDefAtLoc(mediumGazelleID, 0, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));
		else rmPlaceObjectDefAtLoc(mediumZebraID, 0, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));
	}
