...
void main() {
	...
	// RM X Setup.
	rmxInit("Midgard (ESO)", false, false, false);
	...
	if (waterType < 0.5) rmSetSeaType("North Atlantic Ocean");
	else rmSetSeaType("Norwegian Sea");
   ...
   */
	...
	if (rmRandFloat(0, 1) < 0.8) rmAddObjectDefItem(closeBerriesID, "berry bush", rmRandInt(4, 7), 4.0);
	else rmAddObjectDefItem(closeBerriesID, "berry bush", rmRandInt(2, 3), 2.0);
	...
	if (boarNumber < 0.3) rmAddObjectDefItem(closeBoarID, "boar", 2, 4.0);
	else rmAddObjectDefItem(closeBoarID, "boar", 1, 4.0);
	...
	if (predatorSpecies < 0.5) rmAddObjectDefItem(farPredatorID, "wolf", 2, 2.0);
	else rmAddObjectDefItem(farPredatorID, "bear", 1, 1.0);
	...
	if (bonusChance < 0.5) rmAddObjectDefItem(bonusHuntableID, "elk", rmRandInt(4, 9), 4.0);
	else rmAddObjectDefItem(bonusHuntableID, "caribou", rmRandInt(4, 9), 4.0);
	...
	if (cNumberNonGaiaPlayers > 8) rmPlacePlayersCircular(0.3, 0.35, rmDegreesToRadians(5.0));
	else rmPlacePlayersCircular(0.25, 0.3, rmDegreesToRadians(5.0));
	...
	// Settlements.
	id = rmAddFairLoc("Settlement", false, true, 60, 80, 40, 40);
	/*back inside */
	...
	if (bonusChance < 0.7) rmPlaceObjectDefPerPlayer(closeBoarID, false);
	...
	if (rmRandFloat(0, 1) < 0.75) rmPlaceObjectDefPerPlayer(bonusWalrus, false, 1);
	else if (cNumberNonGaiaPlayers < 8) rmPlaceObjectDefPerPlayer(bonusWalrus, false, 2);
	else rmPlaceObjectDefPerPlayer(bonusWalrus, false, 1);
