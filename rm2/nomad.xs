...
void main() {
	...
	for (i = 1; < cNumberPlayers) {
		...
		if (rmGetPlayerCiv(i) == cCivThor) {
			rmAddTriggerEffect("Set Tech Status");
			rmSetTriggerEffectParamInt("PlayerID", i);
			rmSetTriggerEffectParam("TechID", "380");
			rmSetTriggerEffectParam("Status", "0");
		}
	...
	// RM X Setup.
	rmxInit("Nomad (ESO)", false, false, false);
	...
	if (bonusChance < 0.5) rmAddObjectDefItem(bonusHuntableID, "zebra", rmRandFloat(5, 8), 3.0);
	else rmAddObjectDefItem(bonusHuntableID, "giraffe", rmRandFloat(3, 5), 2.0);
	...
	if (bonusChance2 < 0.5) rmAddObjectDefItem(bonusHuntable2ID, "elephant", 2, 2.0);
	else rmAddObjectDefItem(bonusHuntable2ID, "rhinocerous", 2, 2.0);
	...
	for (i = 0; < numTries) {
		...
		if (rmRandFloat(0.0, 1.0) < 0.5) rmSetAreaTerrainType(elevID, "SandA");
	...
	for (i = 0; < cNumberPlayers) {
		if (rmGetPlayerCulture(i) == cCultureGreek) {
			/*            rmPlaceObjectDefAtLoc(VillagerGreekID, i, 0.5, 0.5, 3); */
