...
void main() {
	...
	// RM X Setup.
	rmxInit("Anatolia (ESO)", false, false, false);
	...
	else if (boarNumber < 0.9) rmAddObjectDefItem(closeBoarID, "boar", 4, 4.0);
	...
	if (predatorSpecies < 0.5) rmAddObjectDefItem(farPredatorID, "wolf", 3, 4.0);
	...
	if (bonusChance < 0.5) rmAddObjectDefItem(bonusHuntableID, "deer", 8, 4.0);
	else rmAddObjectDefItem(bonusHuntableID, "deer", 10, 4.0);
	...
	if (cNumberNonGaiaPlayers < 3) rmPlacePlayersLine(0.12, 0.5, 0.88, 0.5);
	...
	for (i = 0; < numTries) {
		...
		if (rmRandFloat(0.0, 1.0) < 0.5) rmSetAreaTerrainType(elevID, "SnowSand50");
	...
	for (i = 0; < forestCount) {
		...
		if (rmRandFloat(0.0, 1.0) < 0.25) rmSetAreaForestType(forestID, "mixed pine forest");
