...
void main() {
	...
	// RM X Setup.
	rmxInit("Valley of Kings (ESO)", false, false, false);
	...
	if (cNumberNonGaiaPlayers < 10) playerConstraint = rmCreateClassDistanceConstraint("stay away from players", classPlayer, 20.0);
	else playerConstraint = rmCreateClassDistanceConstraint("far stay away from players", classPlayer, 30.0);
	...
	if (hippoNumber < 0.3) rmAddObjectDefItem(closeHippoID, "hippo", 2, 1.0);
	else if (hippoNumber < 0.6) rmAddObjectDefItem(closeHippoID, "hippo", 3, 2.0);
	else rmAddObjectDefItem(closeHippoID, "rhinocerous", 2, 1.0);
	...
	else rmAddObjectDefItem(farPredatorID, "hyena", 3, 2.0);
	...
	if (bonusChance < 0.5) rmAddObjectDefItem(bonusHuntableID, "elephant", 2, 4.0);
	else if (bonusChance < 0.75) rmAddObjectDefItem(bonusHuntableID, "giraffe", 3, 4.0);
	else rmAddObjectDefItem(bonusHuntableID, "zebra", 6, 4.0);
	...
	if (bonusChance < 0.5) rmAddObjectDefItem(bonusHuntableID2, "elephant", 1, 2.0);
	else if (bonusChance < 0.75) rmAddObjectDefItem(bonusHuntableID2, "hippo", 2, 2.0);
	else rmAddObjectDefItem(bonusHuntableID2, "hippo", 4, 3.0);
	...
	if (cNumberNonGaiaPlayers < 8) rmSetTeamSpacingModifier(0.25);
	else if (cNumberNonGaiaPlayers < 11) rmSetTeamSpacingModifier(0.40);
	else rmSetTeamSpacingModifier(0.50);
	...
	id = rmAddFairLoc("Settlement A", false, false, 60, 80, 40, 15);
	/* bool forward bool inside */
	if (cNumberNonGaiaPlayers > 8) id = rmAddFairLoc("Settlement B", true, true, 70, 120, 90, 15);
	else id = rmAddFairLoc("Settlement B", true, true, 100, 200, 90, 15);
	...
	if (cNumberNonGaiaPlayers > 8) id2 = rmAddFairLoc("Gold and Migdol", true, true, 70, 100, 50, 15);
	/* bool forward bool inside */
	else id2 = rmAddFairLoc("Gold and Migdol", true, true, 100, 200, 50, 15);
	...
	if (cNumberNonGaiaPlayers < 3) rmPlaceObjectDefAtLoc(farBanditID, 0, 0.5, 0.5, rmRandInt(0, 2));
	else rmPlaceObjectDefAtLoc(farBanditID, 0, 0.5, 0.5, rmRandInt(1, 4));

	// unguarded gold
	if (cNumberNonGaiaPlayers < 3) rmPlaceObjectDefAtLoc(farGoldID, 0, 0.5, 0.5, rmRandInt(2, 3));
	else rmPlaceObjectDefAtLoc(farGoldID, 0, 0.5, 0.5, rmRandInt(3, 6));
	...
	int numPonds2 = 0;
	if (cNumberNonGaiaPlayers < 6) numPonds2 = rmRandInt(2, 4);
	else numPonds2 = rmRandInt(4, 7);
	for (i = 0; < numPonds2) {
	...
	for (i = 0; < cNumberNonGaiaPlayers) {
		...
		if (rmRandFloat(0, 1) < 0.33) rmAddObjectDefItem(statueID, "statue pharaoh", 1, 0.0);
		else if (rmRandFloat(0, 1) < 0.5) rmAddObjectDefItem(statueID, "statue lion left", 1, 0.0);
		else rmAddObjectDefItem(statueID, "statue lion right", 1, 0.0);
