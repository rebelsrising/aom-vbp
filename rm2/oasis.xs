...
void main() {
	...
	// RM X Setup.
	rmxInit("Oasis (ESO)", false, false, false);
	...
	else rmAddObjectDefItem(bonusHuntableID, "giraffe", rmRandInt(2, 5), 2.0);
	...
	if (rmRandFloat(0, 1) < 0.5) rmAddObjectDefItem(farMonkeyID, "baboon", rmRandInt(4, 6), 3.0);
	else rmAddObjectDefItem(farMonkeyID, "monkey", rmRandInt(5, 8), 3.0);
	...
	for (i = 0; < numTries) {
		...
		if (rmRandFloat(0.0, 1.0) < 0.5) rmSetAreaTerrainType(elevID, "SandD");
	...
	else id = rmAddFairLoc("Settlement", false, true, 60, 100, 40, 16);
	...
	if (rmRandFloat(0, 1) < 0.33) rmAddObjectDefItem(deerID, "monkey", rmRandInt(1, 2), 1.0);
	else if (rmRandFloat(0, 1) < 0.5) rmAddObjectDefItem(deerID, "zebra", 1, 0.0);
	else rmAddObjectDefItem(deerID, "gazelle", rmRandInt(1, 3), 2.0);
