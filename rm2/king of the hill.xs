...
void main() {
	...
	// RM X Setup.
	rmxInit("King of the Hill (ESO)", false, false, false);
	...
	if (terrainChance == 0) rmTerrainInitialize("GrassA");
	else if (terrainChance == 2) rmTerrainInitialize("SnowA");
	else rmTerrainInitialize("SandB");
	...
	if (avoidIce == 1) rmAddObjectDefConstraint(startingTowerID, centerConstraint);
	...
	if (avoidIce == 1) rmAddObjectDefConstraint(startingGoldID, centerConstraint);
	...
	if (avoidIce == 1) rmAddObjectDefConstraint(closeChickensID, centerConstraint);

	int closeBoarID = rmCreateObjectDef("close Boar");
	if (terrainChance == 1) rmAddObjectDefItem(closeBoarID, "rhinocerous", 2, 4.0);
	else rmAddObjectDefItem(closeBoarID, "boar", 3, 4.0);
	...
	if (avoidIce == 1) rmAddObjectDefConstraint(closeBoarID, centerConstraint);

	int stragglerTreeID = rmCreateObjectDef("straggler tree");
	if (terrainChance == 1) rmAddObjectDefItem(stragglerTreeID, "oak tree", 1, 0.0);
	if (terrainChance == 1) rmAddObjectDefItem(stragglerTreeID, "palm", 1, 0.0);
	else rmAddObjectDefItem(stragglerTreeID, "pine", 1, 0.0);
	rmSetObjectDefMinDistance(stragglerTreeID, 12.0);
	rmSetObjectDefMaxDistance(stragglerTreeID, 15.0);
	if (avoidIce == 1) rmAddObjectDefConstraint(stragglerTreeID, centerConstraint);
	...
	if (avoidIce == 1) rmAddObjectDefConstraint(mediumGoldID, centerConstraint);
	...
	if (terrainChance == 1) rmAddObjectDefItem(farPredatorID, "lion", 2, 4.0);
	else rmAddObjectDefItem(farPredatorID, "bear", 1, 4.0);
	...
	if (terrainChance == 0) rmAddObjectDefItem(bonusHuntableID, "deer", 6, 3.0);
	else if (terrainChance == 1) rmAddObjectDefItem(bonusHuntableID, "zebra", 6, 3.0);
	else rmAddObjectDefItem(bonusHuntableID, "caribou", 6, 3.0);
	...
	if (terrainChance == 0) rmAddObjectDefItem(randomTreeID, "oak tree", 1, 0.0);
	else if (terrainChance == 1) rmAddObjectDefItem(randomTreeID, "palm", 1, 0.0);
	...
	if (terrainChance == 2) rmAddObjectDefConstraint(randomTreeID, centerConstraint);
	...
	if (terrainChance == 2) {
		...
		if (cNumberNonGaiaPlayers < 3) rmSetAreaSize(centerID, rmAreaTilesToFraction(300), rmAreaTilesToFraction(300));
		else if (cNumberNonGaiaPlayers < 5) rmSetAreaSize(centerID, rmAreaTilesToFraction(1600), rmAreaTilesToFraction(1600));
		if (cNumberNonGaiaPlayers < 7) rmSetAreaSize(centerID, rmAreaTilesToFraction(2200), rmAreaTilesToFraction(2200));
		else rmSetAreaSize(centerID, rmAreaTilesToFraction(2400), rmAreaTilesToFraction(2400));
	...
	for (i = 0; < rmRandInt(3, 6)) {
		...
		if (terrainChance < 2) rmPlaceObjectDefInArea(brokenRuinID, 0, rmAreaID("ruins"), 1);
	}
	...
	for (i = 0; < rmRandInt(4, 8)) {
		...
		if (terrainChance < 2) rmPlaceObjectDefInArea(columnID, 0, rmAreaID("ruins"), 1);
	}
	...
	for (i = 1; < cNumberPlayers * 4) {
		...
		if (terrainChance == 0) rmSetAreaTerrainType(id2, "GrassDirt50");
		else if (terrainChance == 1) {
	...
	for (i = 1; < cNumberPlayers * 8) {
		...
		if (terrainChance == 0) rmSetAreaTerrainType(id3, "GrassDirt25");
		else if (terrainChance == 1) rmSetAreaTerrainType(id3, "SandD");
		else rmSetAreaTerrainType(id3, "SnowB");
	...
	for (i = 1; < cNumberPlayers * 6) {
		...
		if (terrainChance == 2) rmBuildArea(icePatch);
	}
	...
	for (i = 0; < forestCount) {
		...
		if (terrainChance == 0) rmSetAreaForestType(forestID, "oak forest");
		else if (terrainChance == 2) rmSetAreaForestType(forestID, "mixed pine forest");
	...
	else rmAddObjectDefItem(rockID2, "rock granite sprite", 3, 2.0);
	...
	if (terrainChance == 2) rmAddObjectDefConstraint(rockID2, centerConstraint);
