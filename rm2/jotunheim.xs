...
void main() {
	...
	// RM X Setup.
	rmxInit("Jotunheim (ESO)", false, false, false);
	...
	if (huntableNumber < 0.3) rmAddObjectDefItem(closeHuntableID, "deer", 6, 2.0);
	else if (huntableNumber < 0.6) rmAddObjectDefItem(closeHuntableID, "caribou", 4, 2.0);
	else rmAddObjectDefItem(closeHuntableID, "elk", 5, 2.0);
   ...
   rmSetObjectDefMaxDistance(farGoldID, 150.0); */
	...
	if (bonusChance < 0.3) rmAddObjectDefItem(bonusHuntableID, "deer", 6, 2.0);
	else if (bonusChance < 0.6) rmAddObjectDefItem(bonusHuntableID, "elk", 4, 2.0);
	else rmAddObjectDefItem(bonusHuntableID, "caribou", 4, 2.0);
	...
	if (bonusChance3 < 0.3) rmAddObjectDefItem(bonusHuntableID2, "deer", 4, 2.0);
	else if (bonusChance3 < 0.6) rmAddObjectDefItem(bonusHuntableID2, "elk", 6, 2.0);
	else rmAddObjectDefItem(bonusHuntableID2, "caribou", 6, 2.0);
	...
	if (cNumberTeams < 3) {
		if (cNumberNonGaiaPlayers < 4) {
			if (secondConnectionChance < 0.8) secondConnectionExists = 1;
		}
		else {
			if (secondConnectionChance < 0.6) secondConnectionExists = 1;
		}
	}
	...
	if (cNumberNonGaiaPlayers < 4) teamPercentArea = 0.75 / cNumberTeams;
	...
	for (i = 0; < cNumberTeams) {
		...
		if (secondConnectionExists == 1.0) rmAddConnectionArea(alternateConnection, teamID);
	...
	if (secondConnectionExists == 1.0) rmBuildConnection(alternateConnection);
