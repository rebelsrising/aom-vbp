...
void main() {
	...
	// RM X Setup.
	rmxInit("The Unknown (ESO)", false, false, false);
	...
	if (rmRandFloat(0, 1) < 0.33) rmAddObjectDefItem(farPigsID, "pig", rmRandInt(1, 3), 2.0);
	else if (rmRandFloat(0, 1) < 0.50) rmAddObjectDefItem(farPigsID, "goat", rmRandInt(1, 3), 2.0);
	else rmAddObjectDefItem(farPigsID, "cow", rmRandInt(1, 3), 2.0);
	...
	if (teamLands == 1) {
		...
		for (i = 0; < cNumberTeams) {
			...
			if (terrainChance == 0) {
				...
				if (savannah == 1) rmSetAreaTerrainType(teamID, "SavannahB");
				else rmSetAreaTerrainType(teamID, "SandA");
			}
			else if (terrainChance == 2) {
	...
	else rmPlaceObjectDefPerPlayer(relicID, false, 3);
