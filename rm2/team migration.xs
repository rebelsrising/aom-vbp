...
void main() {
	...
	// RM X Setup.
	rmxInit("Team Migration (ESO)", false, false, false);
	...
	if (boarNumber < 0.3) rmAddObjectDefItem(closeBoarID, "hippo", 2, 4.0);
	else if (boarNumber < 0.6) rmAddObjectDefItem(closeBoarID, "hippo", 3, 4.0);
	else rmAddObjectDefItem(closeBoarID, "hippo", 4, 6.0);
	...
	if (predatorSpecies < 0.5) rmAddObjectDefItem(farPredatorID, "lion", 2, 4.0);
	else rmAddObjectDefItem(farPredatorID, "lion", 4, 4.0);
	...
	else if (bonusChance < 0.4) rmAddObjectDefItem(bonusHuntableID, "giraffe", 6, 8.0);
	else if (bonusChance < 0.6) rmAddObjectDefItem(bonusHuntableID, "water buffalo", 4, 4.0);
	else if (bonusChance < 0.8) rmAddObjectDefItem(bonusHuntableID, "rhinocerous", 2, 3.0);
	else rmAddObjectDefItem(bonusHuntableID, "rhinocerous", 3, 3.0);
	...
	if (cNumberNonGaiaPlayers < 4) rmPlacePlayersSquare(0.30, 0.05, 0.05);
	else rmPlacePlayersSquare(0.35, 0.05, 0.05);

	// bonus island
	int centerAvoidance = 0;
	if (cNumberPlayers < 5) centerAvoidance = 30;
	else centerAvoidance = 40;
	...
	if (cNumberTeams == 2) rmAddAreaConstraint(bonusID, teams2StarfishEdgeConstraint);
	else rmAddAreaConstraint(bonusID, teams3StarfishEdgeConstraint);
	...
	if (cNumberNonGaiaPlayers > 3) numCliffs = rmRandInt(2, 4);
	else if (cNumberNonGaiaPlayers > 5) numCliffs = rmRandInt(4, 6);
	for (i = 0; < numCliffs) {
	...
	for (i = 1; < cNumberPlayers) {
		...
		for (j = 0; < num) {
			...
			if (rmRandFloat(0.0, 1.0) < 0.3) rmSetAreaTerrainType(elevID, "GrassDirt50");
	...
	for (j = 0; < num) {
		...
		rmAddAreaConstraint(elevID, avoidBuildings);
		rmAddAreaConstraint(elevID, avoidImpassableLand);
		if (rmRandFloat(0.0, 1.0) < 0.7) rmSetAreaTerrainType(elevID, "GrassDirt50");
		rmSetAreaBaseHeight(elevID, rmRandFloat(3.0, 3.5));
	...
	for (i = 0; < cNumberPlayers) {
		if (rmGetPlayerCulture(i) == cCultureGreek) rmPlaceObjectDefAtLoc(transportGreekID, i, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));
		else if (rmGetPlayerCulture(i) == cCultureNorse) rmPlaceObjectDefAtLoc(transportNorseID, i, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));
		else if (rmGetPlayerCulture(i) == cCultureEgyptian) rmPlaceObjectDefAtLoc(transportEgyptianID, i, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));
		else if (rmGetPlayerCulture(i) == cCultureAtlantean) rmPlaceObjectDefAtLoc(transportAtlanteanID, i, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));
	}
   ...
   rmPlaceObjectDefInRandomAreaOfClass(farCrocsID, false, classBonusIsland, cNumberNonGaiaPlayers); */
	...
	if (rmRandFloat(0, 1) < 0.5) rmAddObjectDefItem(sharkID, "whale", 1, 0.0);
	...
	for (i = 1; < cNumberPlayers) {
		...
		for (j = 0; < forestCount) {
			...
			rmSetAreaWarnFailure(forestID, false);
			if (rmRandFloat(0.0, 1.0) < 0.5) rmSetAreaForestType(forestID, "palm forest");
			...
			// Hill trees?
			if (rmRandFloat(0.0, 1.0) < 0.2) rmSetAreaBaseHeight(forestID, rmRandFloat(3.0, 4.0));
