...
void main() {
	...
	// RM X Setup.
	rmxInit("Molde", false, false, false);
	...
	int shortAvoidSettlement = rmCreateTypeDistanceConstraint("objects avoid TC by short distance", "AbstractSettlement", 15.0);
	...
	// Gold
	int avoidGold = rmCreateTypeDistanceConstraint("avoid gold", "gold", 27.0);
	int shortAvoidGold = rmCreateTypeDistanceConstraint("short avoid gold", "gold", 7.0);
	...
	rmSetObjectDefMaxDistance(startingGoldID, 23.0);
	...
	rmAddObjectDefItem(closeGoatsID, "goat", 2, 5.0);
	...
	if (rmRandFloat(0, 1) < 0.5) rmAddObjectDefItem(closeChickensID, "chicken", rmRandInt(5, 8), 5.0);
	else rmAddObjectDefItem(closeChickensID, "berry bush", rmRandInt(6, 8), 4.0);
	...
	rmAddObjectDefItem(closeBoarID, "Caribou", rmRandInt(6, 8), 9.0);
	...
	rmAddObjectDefItem(stragglerTreeID, "tree snow pine", 1, 0.0);
	...
	if (predatorSpecies < 0.5) rmAddObjectDefItem(farPredatorID, "polar bear", 2, 4.0);
	else rmAddObjectDefItem(farPredatorID, "Wolf", 4, 4.0);
	rmSetObjectDefMinDistance(farPredatorID, 60.0);
	rmSetObjectDefMaxDistance(farPredatorID, 120.0);
	...
	rmAddObjectDefItem(farBerriesID, "polar bear", rmRandInt(0, 1), 9.0);
	rmAddObjectDefItem(farBerriesID, "berry bush", rmRandInt(6, 8), 7.0);
	...
	if (bonusChance < 0.5) {
		rmAddObjectDefItem(bonusHuntableID, "boar", rmRandInt(2, 4), 10.0);
		rmAddObjectDefItem(bonusHuntableID, "deer", rmRandInt(5, 8), 9.0);
	}
	else rmAddObjectDefItem(bonusHuntableID, "Aurochs", rmRandInt(2, 3), 8.0);
	...
	rmAddObjectDefItem(randomTreeID, "pine snow", 4, 19.0);
	...
	rmAddObjectDefItem(randomTreeID2, "pine snow", 4, 19.0);
	rmSetObjectDefMinDistance(randomTreeID2, 0.0);
	rmSetObjectDefMaxDistance(randomTreeID2, rmXFractionToMeters(1.0));
	...
	if (rmRandFloat(0, 1) < 0.5) {
		rmAddObjectDefItem(farMonkeyID, "elk", rmRandInt(5, 9), 15.0);
		rmAddObjectDefItem(farMonkeyID, "wolf", rmRandInt(0, 1), 1.0);
	} else {
		rmAddObjectDefItem(farMonkeyID, "Caribou", rmRandInt(6, 12), 17.0);
		rmAddObjectDefItem(farMonkeyID, "wolf", rmRandInt(1, 2), 7.0);
	}
	...
	rmAddObjectDefItem(relicID, "milestone", 3, 6.0);
	...
	rmPlacePlayersSquare(0.3, 0.06, 0.06);
	...
	for (i = 1; < cNumberPlayers) {
		...
		// Set the size.
		rmSetAreaSize(id, rmAreaTilesToFraction(115), rmAreaTilesToFraction(115));
	...
	for (i = 1; < cNumberPlayers) {
		...
		rmAddAreaTerrainLayer(id, "SnowA", 0, 1);
	}
	...
	for (i = 0; < numTries) {
		...
		if (rmRandFloat(0.0, 1.0) < 0.5) rmSetAreaTerrainType(elevID, "SnowGrass25");
	...
	for (i = 0; < numTries) {
		...
		rmSetAreaSize(elevID, rmAreaTilesToFraction(25), rmAreaTilesToFraction(140));
		...
		if (rmRandFloat(0.0, 1.0) < 0.5) rmSetAreaTerrainType(elevID, "SnowA");
		rmSetAreaMinBlobDistance(elevID, 20.0);
		rmSetAreaMaxBlobDistance(elevID, 40.0);
		rmSetAreaCoherence(elevID, 0.0);
		rmAddAreaConstraint(elevID, centerConstraint);
		rmAddAreaConstraint(elevID, avoidImpassableLand);
	...
	// Settlements.
	id = rmAddFairLoc("Settlement", false, true, 60, 80, 50, 16);
	/* forward inside */
	rmAddFairLocConstraint(id, wideCenterConstraint);

	if (rmRandFloat(0, 1) < 0.75) id = rmAddFairLoc("Settlement", true, false, 70, 120, 50, 16);
	else id = rmAddFairLoc("Settlement", true, false, 60, 100, 40, 16);
	...
	for (i = 0; < numTries) {
		...
		rmSetAreaSize(rockyID, rmAreaTilesToFraction(60), rmAreaTilesToFraction(200));
		rmSetAreaWarnFailure(rockyID, false);
		rmSetAreaMinBlobs(rockyID, 1);
		rmSetAreaMaxBlobs(rockyID, 3);
		rmSetAreaTerrainType(rockyID, "IceC");
		rmAddAreaTerrainLayer(rockyID, "IceA", 0, 2);
		rmSetAreaBaseHeight(rockyID, rmRandFloat(0.0, 2.0));
		rmSetAreaMinBlobDistance(rockyID, 13.0);
		rmSetAreaMaxBlobDistance(rockyID, 23.0);
		...
		rmAddAreaConstraint(rockyID, forestConstraint);
		rmAddAreaConstraint(rockyID, avoidGold);
		rmAddAreaConstraint(rockyID, playerCenterConstraint);
	...
	// Monkeys
	if (rmRandFloat(0, 1) < 0.66) rmPlaceObjectDefPerPlayer(farMonkeyID, false, 1);
	...
	for (i = 0; < forestCount) {
		...
		rmSetAreaSize(forestID, rmAreaTilesToFraction(45), rmAreaTilesToFraction(110));
		...
		rmSetAreaMinBlobDistance(forestID, 25.0);
		rmSetAreaMaxBlobDistance(forestID, 50.0);
	...
	if (rmRandFloat(0, 1) < 0.33) rmAddObjectDefItem(deerID, "Aurochs", rmRandInt(1, 2), 1.0);
	else if (rmRandFloat(0, 1) < 0.5) rmAddObjectDefItem(deerID, "elk", 1, 0.0);
	else rmAddObjectDefItem(deerID, "wolf", rmRandInt(1, 2), 2.0);
	...
	rmPlaceObjectDefAtLoc(rockID, 0, 0.5, 0.5, 20 * cNumberNonGaiaPlayers);

	// Bushes
	int bushID = rmCreateObjectDef("big bush patch");
	rmAddObjectDefItem(bushID, "rotting log", 1, 3.0);
	rmSetObjectDefMinDistance(bushID, 0.0);
	rmSetObjectDefMaxDistance(bushID, rmXFractionToMeters(1.0));
	rmAddObjectDefConstraint(bushID, avoidAll);
	rmAddObjectDefConstraint(bushID, centerConstraint);
	rmPlaceObjectDefAtLoc(bushID, 0, 0.5, 0.5, 1 * cNumberNonGaiaPlayers);

	int bush2ID = rmCreateObjectDef("small bush patch");
	rmAddObjectDefItem(bush2ID, "rock limestone big", 2, 18.0);
	rmAddObjectDefItem(bush2ID, "rock granite sprite", 1, 2.0);
	...
	rmAddObjectDefItem(grassID, "rock limestone sprite", 1, 0.0);
	...
	rmPlaceObjectDefAtLoc(grassID, 0, 0.5, 0.5, 15 * cNumberNonGaiaPlayers);
	...
	rmAddObjectDefItem(sandDrift, "runestone", 1, 0.0);
