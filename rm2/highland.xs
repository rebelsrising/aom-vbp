...
void main() {
	...
	// RM X Setup.
	rmxInit("Highland (ESO)", false, false, false);
	...
	if (hippoNumber < 0.3) rmAddObjectDefItem(closeHippoID, "water buffalo", 2, 1.0);
	else if (hippoNumber < 0.6) rmAddObjectDefItem(closeHippoID, "water buffalo", 3, 4.0);
	...
	else rmAddObjectDefItem(farPredatorID, "bear", 2, 2.0);
	...
	else rmAddObjectDefItem(bonusHuntableID, "water buffalo", rmRandInt(3, 5), 3.0);
	...
	if (bonusChance < 0.5) rmAddObjectDefItem(bonusHuntableID2, "water buffalo", 2, 2.0);
	...
	bonusChance = rmRandFloat(0, 1);
	if (bonusChance < 0.5) rmAddObjectDefItem(bonusHuntableID3, "boar", 3, 2.0);
	else if (bonusChance < 0.75) {
		rmAddObjectDefItem(bonusHuntableID3, "deer", rmRandInt(4, 6), 3.0);
		if (rmRandFloat(0, 1) < 0.5) rmAddObjectDefItem(bonusHuntableID3, "boar", rmRandInt(2, 3), 4.0);
	}
	else rmAddObjectDefItem(bonusHuntableID3, "water buffalo", 4, 3.0);
	...
	rmAddConnectionTerrainReplacement(shallowsID, "RiverGrassyA", "GrassDirt25");
	/*riverSandyC */
	...
	for (i = 1; < cNumberPlayers) {
		...
		if (cNumberNonGaiaPlayers > 2) rmAddAreaConstraint(id, centerConstraint);
		rmSetAreaLocPlayer(id, i);
	...
	// Home Settlement
	id = rmAddFairLoc("Settlement", false, true, 40, 100, 40, 16, true);
