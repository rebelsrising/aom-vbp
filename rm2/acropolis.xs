...
void main() {
	...
	// RM X Setup.
	rmxInit("Acropolis (ESO)", false, false, false);
	...
	int stayInCenter = 0;
	if (cNumberNonGaiaPlayers < 4) rmCreateBoxConstraint("far avoid edge of map", rmXTilesToFraction(30), rmZTilesToFraction(30), 1.0 - rmXTilesToFraction(30), 1.0 - rmZTilesToFraction(30));
	else rmCreateBoxConstraint("medium avoid edge of map", rmXTilesToFraction(20), rmZTilesToFraction(20), 1.0 - rmXTilesToFraction(20), 1.0 - rmZTilesToFraction(20));
	...
	for (i = 1; < cNumberPlayers * 12) {
		...
		if (cNumberNonGaiaPlayers > 5) rmSetAreaSize(patch, rmAreaTilesToFraction(50), rmAreaTilesToFraction(150));
		else rmSetAreaSize(patch, rmAreaTilesToFraction(90), rmAreaTilesToFraction(200));
	...
	// Settlements.
	id = rmAddFairLoc("Settlement", false, false, 60, 140, 60, 40);
	/* forward outside */
	...
	for (i = 1; < cNumberPlayers) {
		...
		if (rmRandFloat(0.0, 1.0) < 0.7) rmSetAreaForestType(playerForestID, "oak forest");
		else rmSetAreaForestType(playerForestID, "autumn oak forest");
	...
	for (i = 0; < numTries) {
		...
		if (rmRandFloat(0.0, 1.0) < 0.7) rmSetAreaForestType(forestID, "oak forest");
		...
		// Hill trees?
		if (rmRandFloat(0.0, 1.0) < 0.3) rmSetAreaBaseHeight(forestID, rmRandFloat(3.0, 4.0));
	...
	if (lake == 1) rmPlaceObjectDefAtLoc(fishID, 0, 0.5, 0.5, rmRandInt(0, 4));
	...
	if (lake == 1) rmPlaceObjectDefAtLoc(decorationID, 0, 0.5, 0.5, 2 * cNumberNonGaiaPlayers);
