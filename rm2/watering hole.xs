...
void main() {
	...
	// RM X Setup.
	rmxInit("Watering Hole (ESO)", false, false, false);
	...
	else rmAddObjectDefItem(farPredatorID, "lion", 3, 2.0);
	...
	if (bonusChance < 0.5) rmAddObjectDefItem(bonusHuntableID, "elephant", 2, 2.0);
	else if (bonusChance < 0.75) rmAddObjectDefItem(bonusHuntableID, "giraffe", rmRandInt(3, 4), 3.0);
	...
	if (bonusChance < 0.5) rmAddObjectDefItem(bonusHuntableID2, "elephant", 2, 2.0);
	else if (bonusChance < 0.75) {
		rmAddObjectDefItem(bonusHuntableID2, "water buffalo", rmRandInt(5, 6), 3.0);
		if (rmRandFloat(0, 1) < 0.5) rmAddObjectDefItem(bonusHuntableID2, "zebra", rmRandInt(2, 4), 3.0);
	}
	else rmAddObjectDefItem(bonusHuntableID2, "gazelle", rmRandInt(6, 9), 4.0);
	...
	bonusChance = rmRandFloat(0, 1);
	if (bonusChance < 0.5) rmAddObjectDefItem(bonusHuntableID3, "hippo", 3, 2.0);
	else if (bonusChance < 0.75) {
		rmAddObjectDefItem(bonusHuntableID3, "zebra", rmRandInt(4, 6), 3.0);
		if (rmRandFloat(0, 1) < 0.5) rmAddObjectDefItem(bonusHuntableID3, "giraffe", rmRandInt(2, 4), 4.0);
	}
	else rmAddObjectDefItem(bonusHuntableID3, "rhinocerous", 4, 3.0);
	...
	for (i = 0; < numTries1) {
		...
		if (rmRandFloat(0.0, 1.0) < 0.7) rmSetAreaTerrainType(elevID, "SavannahC");
	...
	// Home Settlement
	id = rmAddFairLoc("Settlement", false, true, 60, 100, 40, 16, true);
	...
	for (i = 0; < forestCount) {
		...
		if (rmRandFloat(0, 1) < 0.8) rmSetAreaForestType(forestID, "savannah forest");
		else rmSetAreaForestType(forestID, "palm forest");
